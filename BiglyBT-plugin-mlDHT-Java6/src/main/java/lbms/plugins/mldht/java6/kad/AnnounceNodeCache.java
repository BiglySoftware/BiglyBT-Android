...
public class AnnounceNodeCache {
	...
	ConcurrentSkipListMap<Key, CacheBucket> cache = new ConcurrentSkipListMap<Key, CacheBucket>();
	...
	public List<KBucketEntry> get(Key target, int count, Set<KBucketEntry> toExclude)
	{
		...
		Entry<Key, CacheBucket> ceil = cache.ceilingEntry(target);
		Entry<Key, CacheBucket> floor = cache.floorEntry(target);
		if(ceil != null)
