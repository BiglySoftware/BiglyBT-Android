...
public final class HttpHeaders
{
	...
	public static long lastValidLengthForDebug = -1;
	 ...
	 */
	public static long FORCE_UNKNOWN_CONTENT_LENGTH_AT = -1;
	...
	public static long contentLength(Headers headers) {
		long size = stringToLong(headers.get("Content-Length"));
		if (size >= 0) {
			lastValidLengthForDebug = size;
		}
		if (FORCE_UNKNOWN_CONTENT_LENGTH_AT >= 0
				&& size >= FORCE_UNKNOWN_CONTENT_LENGTH_AT) {
			return -1;
		}
		return size;
	}
	...
	public static boolean varyMatches(Response cachedResponse,
			Headers cachedRequest, Request newRequest) {
	...
	public static List<Challenge> parseChallenges(Headers responseHeaders,
		...
		List<String> authenticationHeaders = responseHeaders.values(
				challengeHeader);
	...
	public static void receiveHeaders(CookieJar cookieJar, HttpUrl url,
			Headers headers) {
