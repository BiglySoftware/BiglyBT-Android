	...
	extends CompoundButton
{
	...
	public SwitchCompat(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		ColorStateList thumbTintList = a.getColorStateList(
				R.styleable.SwitchCompat_thumbTint);
		if (thumbTintList != null) {
			mThumbTintList = thumbTintList;
			mHasThumbTint = true;
		}
		PorterDuff.Mode thumbTintMode = parseTintMode(
		...
		ColorStateList trackTintList = a.getColorStateList(
				R.styleable.SwitchCompat_trackTint);
		if (trackTintList != null) {
			mTrackTintList = trackTintList;
			mHasTrackTint = true;
		}
		PorterDuff.Mode trackTintMode = parseTintMode(
	...
	public void setSwitchTextAppearance(Context context, int resid) {
		final TintTypedArray appearance = TintTypedArray.obtainStyledAttributes(
		...
		styleIndex = appearance.getInt(R.styleable.TextAppearance_android_textStyle,
				-1);

		setSwitchTypefaceByIndex(typefaceIndex, styleIndex);

		boolean allCaps = appearance.getBoolean(
				R.styleable.TextAppearance_textAllCaps, false);
		if (allCaps) {
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mThumbDrawable != null) {
			...
			thumbWidth = mThumbDrawable.getIntrinsicWidth() - padding.left
					- padding.right;
		...
		if (measuredHeight < switchHeight) {
			setMeasuredDimension(ViewCompat.getMeasuredWidthAndState(this),
					switchHeight);
		}
	}
	...
	private Layout makeLayout(CharSequence text) {
		final CharSequence transformed = (mSwitchTransformationMethod != null)
				? mSwitchTransformationMethod.getTransformation(text, this) : text;

		return new StaticLayout(transformed, mTextPaint, transformed != null
	...
	private void animateThumbToCheckedState(final boolean newCheckedState) {
		...
		mPositionAnimator = new ThumbAnimation(mThumbPosition,
				newCheckedState ? 1f : 0f);
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (ViewUtils.isLayoutRtl(this)) {
			...
			switchRight = switchLeft + mSwitchWidth - opticalInsetLeft
					- opticalInsetRight;
		} else {
			switchRight = getWidth() - getPaddingRight() - opticalInsetRight;
			switchLeft = switchRight - mSwitchWidth + opticalInsetLeft
					+ opticalInsetRight;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (switchText != null) {
			...
			final int top = (switchInnerTop + switchInnerBottom) / 2
